...
void manipulatorQpController::initializeOptimizer_(int dofs){
	...
	for (int ii = 0; ii< robotPtr_->getNumDofs(); ii++){
		...
		std::shared_ptr<jointPositionLimitConstraint> tempJointUpperPositionConstraintPtr  = std::make_shared<jointPositionLimitConstraint>(robotPtr_,ii, false);
		...
		std::shared_ptr<jointPositionLimitConstraint> tempJointLowerPositionConstraintPtr  = std::make_shared<jointPositionLimitConstraint>(robotPtr_,ii, true);
	...
	for (int ii = 0; ii< robotPtr_->getNumDofs(); ii++){
		std::shared_ptr<jointVelocityLimitConstraint> tempJointUpperVelocityConstraintPtr  = std::make_shared<jointVelocityLimitConstraint>(robotPtr_,ii, false);
		...
		// add the lower bound for joint ii 
		std::shared_ptr<jointVelocityLimitConstraint> tempJointLowerVelocityConstraintPtr  = std::make_shared<jointVelocityLimitConstraint>(robotPtr_,ii, true);
	...
	for (int ii = 0; ii< robotPtr_->getNumDofs(); ii++){
		...
		std::shared_ptr<jointAccelerationLimitConstraint> tempJointUpperAccelerationConstraintPtr  = std::make_shared<jointAccelerationLimitConstraint>(robotPtr_,ii, false, upperBound);
		...
		std::shared_ptr<jointAccelerationLimitConstraint> tempJointLowerAccelerationConstraintPtr  = std::make_shared<jointAccelerationLimitConstraint>(robotPtr_,ii, true, lowerBound);
	...
	for (int ii = 0; ii< robotPtr_->getNumDofs(); ii++){
		...
		std::shared_ptr<jointTorqueLimitConstraint> tempJointUpperTorqueConstraintPtr  = std::make_shared<jointTorqueLimitConstraint>(robotPtr_,ii, false, upperBound);
		...
		std::shared_ptr<jointTorqueLimitConstraint> tempJointLowerTorqueConstraintPtr  = std::make_shared<jointTorqueLimitConstraint>(robotPtr_,ii, true, lowerBound);
