...
{
public:
  orientationTask(const std::string & endEffectorName,
                        ...
                        selectionV)
  {
    ...
    std::cout << "The desired quaternion is: " << std::endl
              << desiredOrientation.w() << ", " << desiredOrientation.vec().transpose() << std::endl;
    ...
    std::cout << "The current quaternion is: " << std::endl
              << currentQuat.w() << ", " << currentQuat.vec().transpose() << std::endl;
    calcQuatError(currentQuat);

    std::cout << "The quaternion error is: " << std::endl
              << getErrorQuaternion().w() << ", " << getErrorQuaternion().vec().transpose() << std::endl;

    // quatScalarWeightingMatrix_.resize(4,4);
    // quatScalarWeightingMatrix_.setIdentity();
    // quatScalarWeightingMatrix_(0, 0) =
    // configurationDataTree_.get<double>("qpController.orientationTask.quaternion_scalar_weight", 0);
